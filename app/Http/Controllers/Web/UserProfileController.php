...
class UserProfileController extends Controller
{
    ...
    public function edit_user_account(Request $request)
    {
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');
        $customerDetail = User::where('id', auth('customer')->id())->first();
        return view(VIEW_FILE_NAMES['edit_user_account'], compact('customerDetail'));
    ...
    public function getUserProfileUpdate(CustomerProfileUpdateRequest $request)//: RedirectResponse
    {
        ...
        User::find(auth('customer')->id())->update([
            ...
            'phone' => $request['phone'],
            'city' => $request['city'],
            'street_address' => $request['area'],
            'house_no' => $request['shipping_address'],
            'image' => $imageName,
    ...
    public function account_address_view()
    {
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');
        $zip_restrict_status = Helpers::get_business_settings('delivery_zip_code_area_restriction');
        $default_location = Helpers::get_business_settings('default_location');
        ...
        return view(VIEW_FILE_NAMES['account_address_view'], compact('countries', 'zip_restrict_status', 'zip_codes', 'default_location'));
    }
    ...
    public function account_address()//: View|RedirectResponse
    {
        ...
        $customer_detail = User::find(auth('customer')->id());
        ...
        if (auth('customer')->check()) {
            ...
            return view('web-views.users-profile.account-address', compact('shippingAddresses', 'country_restrict_status', 'zip_restrict_status', 'countries', 'zip_codes', 'countriesName', 'countriesCode', 'customer_detail'));
        } else {
            return redirect()->route('home');
        }
    }
    ...
    public function user_password(Request $request)
    {
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');
        $customerDetail = User::where('id', auth('customer')->id())->first();
        return view(VIEW_FILE_NAMES['user_password'], compact('customerDetail'));
    ...
    public function user_password_update(Request $request)
    {
        ...
        if (Hash::check($request->current_password, $user->password))
        {
            ...
            Toastr::error(translate('Current password does not match!'));
        }
    ...
    public function gift_cards(Request $request)
    {
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');
        $customerDetail = User::where('id', auth('customer')->id())->first();
        return view(VIEW_FILE_NAMES['gifts_cards'], compact('customerDetail'));
    ...
    public function preferences(Request $request)
    {
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');
        $customerDetail = User::where('id', auth('customer')->id())->first();
        return view(VIEW_FILE_NAMES['preferences'], compact('customerDetail'));
